...
Summing up, in a CQRS application, the write-side and the read-side operate {reactive-principles}/principles/assert-autonomy.html[autonomously {tab-icon}, window="tab"]. Generally, the write-side provides {reactive-principles}/principles/tailor-consistency.html[strong consistency {tab-icon}, window="tab"] while the read-side provides xref:eventual-consistency.adoc[eventual consistency].
